# Security Audit - Ishtirakati 13.0.0+27
...
> بدأ التدقيق على v10، وثُبتت المعالجات في v11، وأضاف v12 اختبار Firestore Emulator. في v13 أُلغي إنشاء مرايا مفاتيح جديدة خارج Keychain وأضيف ترحيل أحادي الاتجاه لمرايا الإصدارات القديمة.

## 1. الملخص التنفيذي

الوضع العام: **المفاتيح الجديدة Keychain-only، والسجل المحلي AES-256-GCM ويفشل مغلقًا، وقواعد Firestore معزولة باختبار emulator. يبقى فرض App Check ونشر القواعد وفحوص App Store إجراءات لوحة خارجية.**
...
1. **AUD-01 - عالٍ:** عولج في v13 بمنع إنشاء المرآة نهائيًا، وترحيل النسخة القديمة إلى Keychain وحذفها بعد نجاح الكتابة فقط.
...
|---|---|---:|---|---|---|---|
| AUD-01 | التخزين/الأسرار | عالية (CVSS تقريبي 7.1) | `lib/services/secure_data_codec.dart`; `lib/services/subscription_store.dart` | كانت مفاتيح AES وAI قابلة للنسخ إلى SharedPreferences. | يمنع v13 إنشاء أي مرآة؛ يرحّل القديمة إلى Keychain ويحذفها بعد نجاح الكتابة فقط؛ الكتابة تفشل مغلقًا عند تعذر Keychain. | عولج في +27 |
...
**المعالجة المنفذة دون فقدان البيانات**

1. لا ينشئ v13 أي مرآة مفتاح جديدة ولا يعرض خيارًا لإعادة تفعيلها.
2. يبحث فك التشفير في Keychain أولًا ويتحقق AES-GCM/MAC قبل قبول المفتاح.
3. عند غياب Keychain فقط، يقرأ مفتاح v12 القديم ويكتبه إلى Keychain.
4. لا يحذف مفتاح v12 إلا بعد نجاح كتابة Keychain؛ إذا فشلت الكتابة تبقى النسخة القديمة لمحاولة إنقاذ لاحقة.
5. يمنع مسار التشفير والحفظ أي كتابة جديدة ما لم يتوفر مفتاح صالح في Keychain.
6. يرحّل مفتاح AI القديم بالطريقة نفسها، ولا يضع قيمته في حالة التطبيق قبل نجاح Keychain.
7. تغطي اختبارات الانحدار أولوية Keychain، والترحيل، ومنع إنشاء المرآة، وفشل الكتابة المغلق.
...
1. ✅ عولج AUD-01 في v13 بترحيل Keychain آمن واختبارات منع فقدان البيانات.
...
**القرار: جاهز من جهة الكود بعد نجاح بوابات CI، ومشروط بإعدادات اللوحات قبل الإنتاج العام.** لا يوجد fail-open معروف في قفل التطبيق أو Firebase Auth، والمفاتيح الجديدة لا تُحفظ خارج Keychain. يجب قبل App Store التحقق من فرض App Check، نشر قواعد Firestore، قيود مفتاح Firebase، رابط سياسة الخصوصية، وتجربة حذف الحساب على حساب حقيقي.
