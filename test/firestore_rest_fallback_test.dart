...
void main() {
  ...
  test(
    'HTTP 200 confirms creation without exposing the payload in result',
    () async {
  ...
  test(
    'REST GET reads revision and updateTime without exposing backup',
    () async {
  ...
  test('REST update advances revision and uses updateTime precondition', () {
    ...
    final write =
        (body['writes']! as List<Object>).single as Map<String, Object>;
    final update = write['update']! as Map<String, Object>;
    final fields = update['fields']! as Map<String, Object>;
  ...
  test(
    'REST update precondition conflict never overwrites newer data',
    () async {
      final client = MockClient(
        (request) async =>
            http.Response('{"error":{"status":"FAILED_PRECONDITION"}}', 412),
      );
  ...
  test(
    'App Check debug provider is restricted to internal App Check builds',
    () {
      expect(
